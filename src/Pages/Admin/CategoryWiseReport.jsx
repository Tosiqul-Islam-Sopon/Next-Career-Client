...
const CategoryWiseReport = () => {
  ...
  const handleExportPDF = useReactToPrint({
    contentRef: pdfRef,
    documentTitle: "next-career-category-wise-report",
  });

  // Calculate total for percentage
  const totalRecruited = categoryData.reduce(
    (sum, cat) => sum + cat.totalRecruited,
    0
  );
          ...
          <div className="flex items-center">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900">
                Category-wise Recruitment Report
              </h1>
              <p className="text-sm text-gray-500">
                Detailed breakdown of hiring by job category
              </p>
            </div>
          </div>
          <button
              ...
              <div className="mb-8">
                <div className="h-8 w-full bg-gray-100 rounded-lg overflow-hidden flex">
                  {categoryData.map((cat, index) => {
                    const percentage =
                      (cat.totalRecruited / totalRecruited) * 100;
                    return (
                      <div
                        key={index}
                ...
                <div className="mt-2 flex flex-wrap gap-3">
                  {categoryData.map((cat, index) => (
                    <div key={index} className="flex items-center text-xs">
                      <div
                        className={`w-3 h-3 rounded-full ${getColor(index).bg} mr-1`}
                      ></div>
