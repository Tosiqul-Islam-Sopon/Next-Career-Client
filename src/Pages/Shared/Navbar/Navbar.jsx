...
const Navbar = () => {
    ...
    const navLinks = <>
        ...
        <li onClick={() => setLeftDropdownOpen(!leftDropdownOpen)}><NavLink to="/products">Jobs</NavLink></li>
        <li onClick={() => setLeftDropdownOpen(!leftDropdownOpen)}><NavLink to="/postJob">Post a Job</NavLink></li>
    </>
    const navLinksLgForUser = <>
        ...
        <li><NavLink className={"hover:bg-green-400 "} to="/appliedJobs">Applied Jobs</NavLink></li>
    </>

    const navLinksLgForRecruiter = <>
        <li><NavLink className={"hover:bg-green-400 "} to="/">Home</NavLink></li>
        <li><NavLink className={"hover:bg-green-400 "} to="/myPostedJobs">My Jobs</NavLink></li>
        <li><NavLink className={"hover:bg-green-400 "} to="/postJob">Post A Job</NavLink></li>
    </>
        ...
        <div className="navbar bg-gray-800 text-white fixed z-10">
            <div className="navbar-start lg:hidden">
                <div className="dropdown" ref={leftDropdownRef}>
                    ...
                    {
                        leftDropdownOpen && (
                            <ul id="navBarActive2" className="menu menu-sm dropdown-content mt-3 z-[1] p-2 shadow bg-base-100 rounded-box w-52 text-black">
                                {navLinks}
                            </ul>
                        )
                    }
            ...
            <div className="lg:navbar-center navbar-start">
                <a className="btn btn-ghost text-4xl text-green-500 roboto">Next <span className="text-white">Career</span></a>
            </div>
            <div className="navbar-end lg:mr-5">
                <ul id="navBarActive1" className="menu menu-horizontal px-1 hidden lg:flex mr-10 space-x-3">
                    {userRole === "recruiter" ? navLinksLgForRecruiter : navLinksLgForUser}
                </ul>
                {user ? (
                        ...
                        </button>
                        {
                            rightDropdownOpen && (
                                <div className="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg py-2 z-20">
                                    <div className="px-4 py-2 text-xl font-bold text-black border-y-2">{user?.displayName}</div>
                                    <Link to="/dashboard" className="block px-4 py-2 text-gray-700 hover:bg-green-600 hover:text-white" onClick={() => setRightDropdownOpen(false)}>
                                        <button onClick={() => setRightDropdownOpen(!rightDropdownOpen)}>
                                            Profile
                                        </button>
                                    </Link>
                                    <button onClick={handleLogout} className="block w-full text-left px-4 py-2 text-gray-700 hover:bg-green-600 hover:text-white">
                                        Logout
                                    </button>
                                </div>
                            )
