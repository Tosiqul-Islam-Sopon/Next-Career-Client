...
import { Briefcase, Calendar, Building, MapPin, Clock, ChevronRight, Search } from "lucide-react"
...
const AppliedJobs = () => {
  ...
  const filteredJobs = appliedJobs.filter((job) => {
    ...
    const matchesSearch =
      ...
      job.companyInfo?.companyName?.toLowerCase().includes(searchTerm.toLowerCase())
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="flex flex-col items-center">
          ...
          <p className="mt-4 text-lg text-gray-700">Loading your applications...</p>
        </div>
      </div>
    )
  }

  const getStatusBadge = (appliedUser) => {
    if (appliedUser?.rejected) {
    ...
    return <span className="px-3 py-1 text-xs font-medium rounded-full bg-blue-100 text-blue-800">Applied</span>
  }
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 mb-8">
          ...
          <div className="bg-blue-50 rounded-lg p-4 mb-6 border border-blue-100">
            <p className="text-blue-800">
              <span className="font-semibold">{applications.length}</span> job applications found
            </p>
          </div>
          ...
          {filteredJobs.length > 0 ? (
            <div className="space-y-6">
              {filteredJobs.map((job) => {
                ...
                const appliedUser = application.appliedUsers.find((user) => user.userId === userInfo._id)
                ...
                const isRejected = appliedUser?.rejected || false

                return (
                  <div
                    key={job._id}
                    className={`bg-white border ${isRejected ? "border-red-200" : "border-gray-200"} rounded-xl overflow-hidden hover:shadow-md transition-shadow duration-300`}
                  >
                    <div className="p-6">
                      <div className="flex flex-col md:flex-row md:items-center justify-between mb-4">
                        <div className="flex items-start space-x-4">
                          ...
                          <div>
                            ...
                            <p className="text-gray-600">{job.companyInfo?.companyName}</p>
                          </div>
                        </div>
                        <div className="mt-4 md:mt-0">{getStatusBadge(appliedUser)}</div>
                      </div>
                      ...
                      {isRejected && (
                        <div className="mb-4 p-3 bg-red-50 border border-red-100 rounded-lg">
                          <p className="text-red-700 text-sm">
                            Your application has been rejected. We encourage you to apply for other opportunities.
                          </p>
                        </div>
                      )}
            ...
            <div className="text-center py-16">
              ...
              <h3 className="text-lg font-medium text-gray-900 mb-1">No matching applications found</h3>
